...
<script>
document.addEventListener('DOMContentLoaded', function() {
    window.initRolesIndex({
        routes: {
            index: '{{ route('roles.index') }}',
            permissions: '{{ route('roles.edit_permissions', ':id') }}',
            edit: '{{ route('roles.edit', ':id') }}',
            destroy: '{{ route('roles.destroy', ':id') }}',
            can_edit: @json(auth()->user()->hasPermission('roles.edit')),
            can_delete: @json(auth()->user()->hasPermission('roles.destroy')),
            can_perms: @json(auth()->user()->hasPermission('roles.edit'))
        },
        tokens: {
            csrf: '{{ csrf_token() }}'
        }
