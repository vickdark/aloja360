                                ...
                                <select name="accommodation_id" id="accommodation_id" class="form-select form-select-lg" required>
                                    @foreach($accommodations as $acc)
                                        <option value="{{ $acc->id }}"
                                            ...
                                            {{ old('accommodation_id', $ratePeriod->accommodation_id) == $acc->id ? 'selected' : '' }}>{{ $acc->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label for="is_weekend" class="w-100 mb-0" style="cursor:pointer;">
                                    <input class="d-none" type="checkbox" name="is_weekend" id="is_weekend" value="1" {{ old('is_weekend', $ratePeriod->is_weekend) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                <label for="is_holiday" class="w-100 mb-0" style="cursor:pointer;">
                                    <input class="d-none" type="checkbox" name="is_holiday" id="is_holiday" value="1" {{ old('is_holiday', $ratePeriod->is_holiday) ? 'checked' : '' }}>
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    ...
                                    <select name="accommodation_adjustment_type" id="accommodation_adjustment_type" class="form-select">
                                        <option value="" {{ old('accommodation_adjustment_type', $ratePeriod->accommodation_adjustment_type) === null ? 'selected' : '' }}>Usar adulto</option>
                                        <option value="amount" {{ old('accommodation_adjustment_type', $ratePeriod->accommodation_adjustment_type) == 'amount' ? 'selected' : '' }}>Monto fijo (+ $)</option>
                                        <option value="percentage" {{ old('accommodation_adjustment_type', $ratePeriod->accommodation_adjustment_type) == 'percentage' ? 'selected' : '' }}>Porcentaje (+ %)</option>
                                    </select>
                                </div>
                                <div class="col-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" step="100" min="0" name="accommodation_adjustment_value" id="accommodation_adjustment_value" value="{{ old('accommodation_adjustment_value', $ratePeriod->accommodation_adjustment_value) }}" class="form-control" placeholder="Vacío = adulto">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    ...
                                    <select name="adjustment_type" id="adjustment_type" class="form-select">
                                        <option value="amount" {{ old('adjustment_type', $ratePeriod->adjustment_type ?? 'amount') == 'amount' ? 'selected' : '' }}>Monto fijo (+ $)</option>
                                        <option value="percentage" {{ old('adjustment_type', $ratePeriod->adjustment_type) == 'percentage' ? 'selected' : '' }}>Porcentaje (+ %)</option>
                                    </select>
                                </div>
                                <div class="col-6">
                                    ...
                                    <div class="input-group" id="adjustment_value_group">
                                        ...
                                        <input type="number" step="100" min="0" name="adjustment_value" id="adjustment_value" value="{{ old('adjustment_value', $ratePeriod->adjustment_value) }}" class="form-control" required>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    ...
                                    <select name="child_adjustment_type" id="child_adjustment_type" class="form-select">
                                        <option value="" {{ old('child_adjustment_type', $ratePeriod->child_adjustment_type) === null ? 'selected' : '' }}>Usar adulto</option>
                                        <option value="amount" {{ old('child_adjustment_type', $ratePeriod->child_adjustment_type) == 'amount' ? 'selected' : '' }}>Monto fijo (+ $)</option>
                                        <option value="percentage" {{ old('child_adjustment_type', $ratePeriod->child_adjustment_type) == 'percentage' ? 'selected' : '' }}>Porcentaje (+ %)</option>
                                    </select>
                                </div>
                                <div class="col-6">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text">$</span>
                                        <input type="number" step="100" min="0" name="child_adjustment_value" id="child_adjustment_value" value="{{ old('child_adjustment_value', $ratePeriod->child_adjustment_value) }}" class="form-control" placeholder="Vacío = adulto">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-3 bg-light rounded-4 border mb-3">
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text">$</span>
                                        <input type="number" step="100" min="0" name="extra_guest_price" value="{{ old('extra_guest_price', $ratePeriod->extra_guest_price) }}" class="form-control">
                                    </div>
                                </div>
                                <div class="col-6">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text">$</span>
                                        <input type="number" step="100" min="0" name="extra_child_price" value="{{ old('extra_child_price', $ratePeriod->extra_child_price) }}" class="form-control" placeholder="Vacío = adulto">
                                    </div>
                                </div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateReference(){
        ...
        if (!opt || !opt.value) { refCard.style.display='none'; return; }
    ...
    function updatePreview(){
        const opt = accSelect.options[accSelect.selectedIndex];
        if(!opt || !opt.value){ previewAlert.style.display='none'; return; }
        const base = parseFloat(opt.getAttribute('data-base-price'))||0;
        const adult = parseFloat(opt.getAttribute('data-price-per-person'))||0;
